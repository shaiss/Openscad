...
| N2 | **No RUN of continuously enclosed bore exceeds 2 × body length.** A *run* is the maximal chain of enclosed bore between two **breaks**. A break is: an **open module** (longitudinal window ≥ 180°), a **port discharging into a ventilated enclosure**, or a **turnaround node** of clear internal width ≥ body length. **A bend is not a break. A junction at bore diameter is not a break. A coupling is not a break. A top hatch resets RETRIEVAL, not REVERSING.** | **360 mm** per run at `body_len_mm = 180`; **300 mm** if the run is not hand-releasable | Deutscher Tierschutzbund position paper *Tierschutzwidriges Zubehör* — **not TVT Merkblatt 62**; see research §11.2. One limb of a conjunctive test (length **and** ventilation **and** instructions), stated per tube | The owner measures a longer animal — the assert scales, the rule does not. Or someone opens the DTSchB PDF and finds it is per-*system*, in which case the per-run scope is retracted and N2 reverts to a total |
