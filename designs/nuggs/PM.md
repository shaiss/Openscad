...
| N2 | **No RUN of continuously enclosed bore exceeds 2 × body length.** The primary text applies the cap to `Kunststoffröhren`, not to a summed system; NUGGS deliberately applies it more strictly to the whole continuous run. A *run* is the maximal chain of enclosed bore between two **breaks**. A break is: an **open module** (longitudinal window ≥ 180°), a **port discharging into a ventilated enclosure**, or a **turnaround node** of clear internal width ≥ body length **that is itself open to ventilated space** (width alone is not a break — a sealed wide chamber is a dead volume, and N3 forbids it). **A bend is not a break. A junction at bore diameter is not a break. A coupling is not a break. A top hatch resets RETRIEVAL, not REVERSING.** These run/break definitions are NUGGS engineering judgement, not source language. | **360 mm** per run at `body_len_mm = 180`; **min(2 × body_len_mm, 300)** if the run is not hand-releasable — not "drops to 300", which is only a drop above body_len_mm = 150 | Deutscher Tierschutzbund position paper *Tierschutzwidriges Zubehör*, pp. 7–8; [primary PDF, transcript and decision map](../../docs/nuggs-sources/tierschutzwidriges-zubehoer.md). One limb of a conjunctive limit: length **and** ventilation **and** instructions | The owner measures a longer animal — the assert scales, the rule does not |
...
| **2026-08-04** | **Primary-text review resolves N2's source scope as per tube; remove PROVISIONAL** | The complete 12-page DTSchB paper is now committed with a page-addressable transcript. Pages 7–8 make `Kunststoffröhren` the subject of the at-most-2×, ventilation and instructions conditions; no summed system formula appears. NUGGS's per-run cap is deliberately stricter than a per-component reading. The paper contains no 7 cm/70 mm minimum, so N1 keeps its safety floor but may not cite this paper for the number |
