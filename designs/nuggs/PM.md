...
| **B1c** | **A docs-vs-model consistency check** | Five of this design's defects have now been a doc claiming something the model does not do, and four were caught by a human rather than a check. The two that *stopped* recurring — the mate-test clockings and now `port_tol` — were fixed by making the check read the value back out of the geometry. Nothing in this repo gates a *fit* or a *claim*; `ci.parts` only gates parts | Needs a convention that does not exist yet — same blocker as gating the mate test |
