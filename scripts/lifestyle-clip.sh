...
sys.stderr.write("unexpected task_status %r: %s\n" % (status, raw[:800])); sys.exit(5)')" || exit 1
    ...
    # bounded well inside the workflow's 45-minute timeout even after the
    # minutes-long generation poll above). A 3xx is refused immediately and
...
PY
    ...
    if (( ! dl_ok )); then
      echo "video download failed after 5 attempts — the per-attempt lines above carry the diagnosis (issue #128)" >&2
      exit 1
    fi
