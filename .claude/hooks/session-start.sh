...
# (montage, for multi-view preview sheets). Idempotent — exits fast when
# everything is already present (e.g. cached container state).
set -euo pipefail
...
if command -v openscad >/dev/null 2>&1 \
  ...
  && command -v montage >/dev/null 2>&1; then
...
$SUDO apt-get install -y -qq openscad xvfb imagemagick

echo "Installed: $(openscad --version 2>&1)"
