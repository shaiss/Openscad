...
- `docs/` — repo-level research and reference notes: `oss-libraries-research.md` (the OSS-library evaluation behind the adoption backlog) and `derivative-designs.md` (what a derivative is here, the four silent failure modes of OpenSCAD's override mechanism with the measurements behind them, and what each gate proves) — read that one before building on another design.
...
- **Derivative designs.** A design that builds on another one — `include <../<parent>/<parent>.scad>`, then redefine the modules it wants different, so the parent's own call sites route to the redefinition — is a *derivative*, and ships `designs/<name>/derives.conf` recording its lineage (start from `templates/derives.conf`; the background, including the measurements, is `docs/derivative-designs.md`). Four rules follow from the fact that OpenSCAD reports **nothing** when a redefinition binds nothing:
  ...
  - A diamond (two parents sharing an ancestor) evaluates that ancestor twice and unions the duplicate cleanly, so nothing downstream can see it. It fails `lineage check` unless `diamond-ok:` names the ancestor, and that assertion is only true if the ancestor is **base-safe** — its top level defines modules and emits no geometry. The gate proves it by rendering the ancestor and counting facets; no design here qualifies today.
