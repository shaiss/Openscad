...
The SessionStart hook (`.claude/hooks/session-start.sh`) installs the full toolchain: openscad, xvfb, imagemagick, povray (+ povray-includes, for `product-shot.sh`), prusa-slicer, printcheck and stylelift (with pytest). If any of those commands is missing mid-session, run `.claude/hooks/session-start.sh --force` rather than working around the gap (`--force` is needed outside Claude Code on the web, where the hook otherwise no-ops) — `gate.sh --slice` must be runnable locally.
...
- `tools/photoshot/` — the STL → POV-Ray studio renderer behind `product-shot.sh`; has its own README. Needs povray locally, but CI never runs it: the gate only checks that the committed PNGs exist and are embedded.
