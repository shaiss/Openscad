...
4. **Finish.** A design is done when the user approves the preview and `/preflight` comes back green (`check.sh`, `render.sh <name>`, and `gate.sh --slice <name>` all clean). Send the final STL to the user as well — it's the deliverable they'll slice. Then write the product page: copy `templates/README.md` to `designs/<name>/README.md`, fill it in (commit the preview images it shows under `designs/<name>/previews/`), and check it with `./scripts/readme-gate.sh <name>` — CI rejects designs without one.
