...
- `tools/backlog-burn/` — the selection engine behind the **scheduled backlog burn** (`.github/workflows/backlog-burn.yml`, issue #95): the nightly routine that picks one open issue and runs `/ship-issue` on it unattended. The tool owns only the reproducible decision — *which* issue — as a **pure, tested** policy (`select`): at most one issue per firing, filtered to the `autonomy-ok` opt-in label, oldest-first, excluding any issue already claimed by an active `🚢 SHIP-LOCK`, an open closing PR, or a `claude/issue-<N>-*` branch (the same §0 lock signals `/ship-issue` re-verifies). Has its own README and pytest suite with a negative control per guard (CI runs it when the tool changes); stdlib-only like `tools/lineage`, since the scheduled job runs on the system Python. Two safety switches gate the workflow, not the tool: the repo variable **`BACKLOG_BURN_ENABLED`** (unset/`false` pauses the whole routine without deleting it) and the per-issue `autonomy-ok` label (nothing is eligible until a human adds it, so the routine ships nothing until the backlog is curated). The unattended run is invoked via the Claude Code Action and skips with a `::notice::` when its secret is absent, the same way `regen` skips product-page drafting without `ANTHROPIC_API_KEY`.
