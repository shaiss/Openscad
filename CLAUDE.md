...
Anything **derived** from a design is CI's job, not yours. The `regen` job in `.github/workflows/ci.yml` re-runs `render.sh --previews`, `animate.sh`, `product-shot.sh` and `gallery.sh` for every design a PR touches (blast radius included), drafts a product page with `product-page.sh` when one doesn't pass its gate, and commits the result back to the PR branch. Don't run those generators to produce something to commit — run them when you want to *look* at the output.
...
- `designs/<name>/ARCHIVED` — presence marks a design **frozen at v0.1 and retired from active CI** (first line is the version, e.g. `v0.1`; the rest is a human note). CI runs an archived design only when a PR edits its own files: it is skipped by every full-catalog run (the default-branch push and any infra change, where `gate_designs=ALL`) across `gate.sh`, `regen`, `geo-diff` and `check.sh`, and skipped even on a scoped PR that reaches it only through blast radius or a shared style — so it never spends render, slice or product-shot cycles again. A PR that edits the design's own files still gates it normally, so the marker retires a design without stranding it: the revival path is to fork, update against current CI, and contribute the result back as a derivative. See "Archived designs (v0.1 freeze)" under Design conventions.
...
- `scripts/` — the full toolchain: `render.sh`, `check.sh`, `gate.sh`, `lineage.sh`, `readme-gate.sh`, `animate.sh`, `product-shot.sh`, `lifestyle-shot.sh`, `product-page.sh`, `gallery.sh`, `style-lift.sh`, `style-check.sh`, `lint-scad.sh`, `site.sh` (all described above), plus `docs-check.sh` (docs-drift assertions, run by check.sh), `guard-check.sh` (fires every `lib/*-guards.conf` case and fails if a library guard stops refusing it, run by check.sh), `mate-check.sh` (renders every `lib/*-mates.conf` case and fails if a declared fit stops assembling — or if its negative control stops interfering, run by check.sh), `gate-summary.py` (turns a gate.sh log into the markdown table CI posts as the job summary and sticky PR comment) and `preview-budget.sh` (sourced helper defining the GIF and product-shot size budgets). `docs-check.sh` asserts this bullet names every file in `scripts/`, so keep it exhaustive.
