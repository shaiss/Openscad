...
The SessionStart hook (`.claude/hooks/session-start.sh`) installs the full toolchain: openscad, xvfb, imagemagick, bpy (Blender as a Python module, for `product-shot.sh`), prusa-slicer, and printcheck (with pytest). `bpy` is a Python module rather than a command, so `command -v` won't find it — check it with `python3 -c 'import bpy'`. If any of them is missing mid-session, run `.claude/hooks/session-start.sh --force` rather than working around the gap (`--force` is needed outside Claude Code on the web, where the hook otherwise no-ops) — `gate.sh --slice` must be runnable locally.
