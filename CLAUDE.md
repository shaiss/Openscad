...
`bpy` (Blender as a Python module, behind `product-shot.sh`) is **not** installed by default: CI renders and commits the product shots now, so a session doesn't need a ~1 GB wheel to ship a design. Pass `--with-bpy` to the hook if you want to preview a shot locally — `bpy` is a module rather than a command, so `command -v` won't find it; check with `python3 -c 'import bpy'`.
