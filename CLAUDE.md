...
- `scripts/` — the full toolchain: `render.sh`, `check.sh`, `gate.sh`, `lineage.sh`, `readme-gate.sh`, `animate.sh`, `product-shot.sh`, `lifestyle-shot.sh`, `lifestyle-clip.sh`, `product-page.sh`, `gallery.sh`, `style-lift.sh`, `style-check.sh`, `lint-scad.sh`, `site.sh` (all described above), plus `docs-check.sh` (docs-drift assertions, run by check.sh), `guard-check.sh` (fires every `lib/*-guards.conf` case and fails if a library guard stops refusing it, run by check.sh), `mate-check.sh` (renders every `lib/*-mates.conf` case and fails if a declared fit stops assembling — or if its negative control stops interfering, run by check.sh), `gate-summary.py` (turns a gate.sh log into the markdown table CI posts as the job summary and sticky PR comment), `shot-spec.sh` (authors a design's `shots.conf`/`lifestyle.conf` from a PM's art-direction brief — named views and colors instead of camera tuples and hex, the freeze and disclosure standards enforced mechanically; the mechanics behind `/art-direction`, with a `--selftest` run by check.sh), `preview-budget.sh` (sourced helper defining the GIF and product-shot size budgets) and `regen-stamp.sh` (prints a design's regeneration input fingerprint; CI's regen job compares it against the committed `designs/<name>/previews/.regen-stamp` and skips re-rendering a design whose inputs have not moved). `docs-check.sh` asserts this bullet names every file in `scripts/`, so keep it exhaustive.
...
- `tools/backlog-burn/` — the selection engine behind the **scheduled backlog burn** (`.github/workflows/backlog-burn.yml`, issue #95): the weekly routine that picks one open issue and runs `/ship-issue` on it unattended. The tool owns only the reproducible decision — *which* issue — as a **pure, tested** policy (`select`): at most one issue per firing, filtered to the opt-in label (default `autonomy-ok`), oldest-first, excluding any issue already claimed by an active `🚢 SHIP-LOCK`, an open closing PR, or a `claude/issue-<N>-*` branch (the same §0 lock signals `/ship-issue` re-verifies). It also parses the committed policy file (`config`). Has its own README and pytest suite with a negative control per guard (CI runs it when the tool changes); stdlib-only like `tools/lineage`, since the scheduled job runs on the system Python. Config lives in two places, deliberately split: **`.github/backlog-burn.conf`** is the git-tracked source of truth (`enabled`, the opt-in `label`) — the reproducible policy, mirroring `.github/ci-gates/registry.conf`; the repo variable **`BACKLOG_BURN_ENABLED`** is the live arming/kill switch (a fast, human-only toggle not in git). The routine acts only when **both** agree, so a clone/fork can't silently arm it, yet it halts in seconds. Cadence is the `cron` literal in the workflow (Actions can't read a var for `on.schedule`; still git-tracked). The unattended run uses the existing **`ANTHROPIC_API_KEY`** secret via the Claude Code Action (a plain API key — pay-per-token — is the right fit for an unattended bot; no OAuth token needed) and skips with a `::notice::` when that secret is absent, the same way `regen` skips product-page drafting without it.
